...
# NVME_TCP_SOURCE_SHA256 in the source configuration file, which defaults
# to /etc/nvme-tcp-dkms/source.conf.
script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
selector="${script_dir}/select-modules.sh"
config=${NVME_TCP_SOURCE_CONFIG:-/etc/nvme-tcp-dkms/source.conf}
...
case "$role" in
  ...
  target)
    ...
    # Fail fast before any download: the target role must carry patches.
    command -v patch >/dev/null 2>&1 || {
      echo "the patch command is required to apply upstream fixes" >&2
      exit 1
    }
    allowed=' nvmet-tcp '
...
# The target role carries upstream fixes that are applied to the copied
# module sources, whose layout is the flat module directory (tcp.c and
# friends at the root), so patches are applied with -p4. A patch applies
# only when a forward dry-run succeeds; when it is already present in the
# vendor source a reverse dry-run detects it instead. Anything else means
# the source is neither vulnerable nor already fixed and the build stops:
# shipping an unpatched nvmet-tcp is how the queue-install crash returns.
if [[ "$role" == target ]]; then
  patch_dir="${script_dir}/patches"
  mapfile -t patches < <(find "$patch_dir" -maxdepth 1 -type f -name '*.patch' -print 2>/dev/null | sort)
  (( ${#patches[@]} > 0 )) || {
    echo "no upstream fixes found in ${patch_dir}; cannot prepare patched nvmet-tcp sources" >&2
    exit 1
  }
  for patch_file in "${patches[@]}"; do
    patch_name=${patch_file##*/}
    # --batch alone is not enough for the forward check: when hunks look
    # already applied, batch mode assumes -R and the check would succeed.
    # -N keeps the dry-run honest so already-applied hunks fail it.
    if patch --batch -N --dry-run -p4 -d "$build_dir" < "$patch_file" >/dev/null; then
      patch --batch -N --no-backup-if-mismatch -p4 -d "$build_dir" < "$patch_file" || {
        echo "failed to apply ${patch_name} after a successful dry run" >&2
        exit 1
      }
      printf 'applied %s to copied sources\n' "$patch_name"
    elif patch --batch -R --dry-run -p4 -d "$build_dir" < "$patch_file" >/dev/null; then
      printf '%s already present in vendor sources\n' "$patch_name"
    else
      echo "vendor source is incompatible with ${patch_name}: neither vulnerable nor already fixed" >&2
      exit 1
    fi
  done
fi
